...
import com.trueu.titigoface.model.FaceRegisterInfo;
...
public class FaceServer {
        ...
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                if (engineCode == ErrorInfo.MOK) {
                    ...
                    faceEngine = null;
                    Log.e(TAG, "数据:FaceServer init failed! code = " + engineCode);
                    return false;
                }
            }
            return false;
    ...
    public int clearAllFaces(Context context) {
        synchronized (this) {
            ...
            File featureFileDir = new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR);//清除特征库
            ...
            File imgFileDir = new File(ROOT_PATH + File.separator + SAVE_IMG_DIR);//清除图片库
            if (imgFileDir.exists() && imgFileDir.isDirectory()) {
     ...
     * @return
     */
    public void clearFacesByUserNum(Context context, String userNo) {
        synchronized (this) {
            if (context == null) {
                return;
            }
            ...
            if (featureFileDir.exists() && featureFileDir.isDirectory()) {
                ...
                if (featureFiles != null && featureFiles.length > 0) {
                    for (File featureFile : featureFiles) {
                        if (featureFile.getName().contains(userNo)) {
                            featureFile.delete();
                            Log.d("数据", "特征库删除成功！");
                            break;
                        }
            ...
            if (imgFileDir.exists() && imgFileDir.isDirectory()) {
                ...
                if (imgFiles != null && imgFiles.length > 0) {
                    for (File imgFile : imgFiles) {
                        if (imgFile.getName().contains(userNo)) {
                            imgFile.delete();
                            Log.d("数据", "图片库删除成功！");
                            break;
                        }
    ...
    public boolean registerBgr24(Context context, byte[] bgr24, int width, int height, String name) {
        ...
        synchronized (this) {
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                Log.e(TAG, "数据 registerBgr24: no face detected, code is " + code);
                return false;
            }
        }
